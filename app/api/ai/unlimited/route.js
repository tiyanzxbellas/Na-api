...
export async function POST(req) {
    try {
        ...
        reportError(err, { endpoint: '/ai/unlimited', method: 'POST' }).catch(() => {});

                await send({ error: err.message });
            } finally {
                try { await writer.close(); } catch (e) {
        // Auto-report error ke Telegram
        reportError(e, { endpoint: '/ai/unlimited', method: 'UNKNOWN' }).catch(() => {});
}
            }
        })();
        ...
        // Auto-report error ke Telegram
        reportError(error, { endpoint: '/ai/unlimited', method: 'UNKNOWN' }).catch(() => {});
